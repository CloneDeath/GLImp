	...
	namespace Utility {
		namespace MDF {
            ...
            public class MDFChunkReader {
                ...
                public MDFChunkHeader findNextChunk(MDFChunkHeader container, uint id, long start) {
					var origPos = reader.BaseStream.Position;
					var curPos = start + container.offset + sizeof(uint) + sizeof(uint);
					MDFChunkHeader temp;
                ...
                public void enterChunk(MDFChunkHeader header) {
					reader.BaseStream.Seek(header.offset + sizeof(uint) + sizeof(uint), SeekOrigin.Begin);
				}
                ...
                /// </summary>
                /// <returns>the value read.</returns>
                public double readFloat() => reader.ReadSingle();
                ...
                public uint[] readUVIntArray() {
					var size = readUVInt32();
                ...
                public long tell() => reader.BaseStream.Position;
            ...
            public class MDFChunkWriter {
                ...
                public MDFChunkWriter(string filepath) {
					try {
						...
						throw new IOException(
							"new MDFChunkWriter: Could not create or open the given file for writing.", ex);
					}
				}
                ...
                /// <summary>
                ///     Writes the chunk header and returns a new header with an offset field marking the place where the header was
                ///     written.
                /// </summary>
                ...
                /// <summary>
                ///     Calculates the size of the chunk from the current file position back to the start of the chunk header and writes
                ///     that size into the chunk header.
                ...
                public MDFChunkHeader writeCalculatedChunkSize(MDFChunkHeader header) {
					...
					header.size = (uint)(curPos - header.offset);
                ...
                /// </summary>
                /// <param name="val">the value to write.</param>
                public void writeUVInt32(uint val) {
					if (val >= 254) {
						if (val >= 65536) {
                ...
                public long tell() => writer.BaseStream.Position;
